...
def create_dataloader(path,
    ...
    class LoadImagesLabelsAndCl(LoadImagesAndLabels):
        def __init__(self, labelcl_path, *args, **kwargs):
            super().__init__(*args, **kwargs)
            self.labelcl = np.load(labelcl_path, allow_pickle=True).item() if labelcl_path else None
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
                 ...
                 min_items=0,
                 prefix='',
                 cache_path=None):  # Pass cache_path as a parameter
        self.img_size = img_size
        ...
        # Create a cache for cl
        self.create_cl_cache(cache_path)  # Pass cache_path to create_cl_cache
    ...
    def create_cl_cache(self, cache_path):
            cl_cache = {}  # Cache dictionary for cl
            ...
            # Save cl_cache to a separate cache file
    ...
    def load_mosaic(self, index):
        ...
        labels4, segments4 = [], []
        s = self.img_size
        # yc, xc = (int(random.uniform(-x*1.25, 1.75 * s + x)) for x in self.mosaic_border)  # more stable mosaic center x, y
...
#             nf = 1  # label found
#             with open(lb_file) as f:
#                 lb = [x.split() for x in f.read().strip().splitlines()[:-1] if len(x)]
...
#             nf = len(lb_lines)  # count of lines
#             if nf == 1:  # Object detection label
#                 if nf == 1:  # Object detection label
#                 lb = [x.split() for x in lb_lines[0].splitlines() if len(x)]
#                 if any(len(x) > 6 for x in lb):  # is segment
#                     classes = np.array([x[0] for x in lb], dtype=np.float32)
#                     segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in lb]  # (cls, xy1...)
#                     lb = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh)
#                 else:
#                     lb = np.array(lb, dtype=np.float32)
#             elif nf == 2:  # Image classification label
#                 lb = np.array([list(map(float, x.split())) for x in lb_lines], dtype=np.float32)
#             else:
#                 raise ValueError("Invalid label file format. Expected either one line (object detection) or two lines (object detection and image classification).")
...
#                         msg = f'{prefix}WARNING ⚠️ {im_file}: {nl - len(i)} duplicate labels removed'
#                 elif nf == 2:  # Image classification label
#                     assert all(0 <= label <= 1 for label in lb), f'Classification labels should be between 0 and 1'
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                lines = f.read().strip().splitlines()
                lb_lines = lines[:-1]  # All lines except the last one
                cl_line = lines[-1]    # Last line
                lb = [x.split() for x in lb_lines if len(x)]
                if any(len(x) > 6 for x in lb):  # is segment
                    classes = np.array([x[0] for x in lb], dtype=np.float32)
                    segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in lb]  # (cls, xy1...)
                    lb = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh)
                ...
                cl = [cl_line.split()]
                if any(len(x) > 4 for x in cl):  # is segment
                    classes = np.array([x[0] for x in cl], dtype=np.float32)
                    segments = [np.array(x[1:], dtype=np.float32).reshape(-1, 2) for x in cl]  # (cls, xy1...)
                    cl = np.concatenate((classes.reshape(-1, 1), segments2boxes(segments)), 1)  # (cls, xywh)
