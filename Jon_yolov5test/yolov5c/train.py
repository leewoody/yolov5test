...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for layer in de_parallel(model).model:
        if isinstance(layer, Detect):
            detect_layer = layer
            break  # stop after finding the first Detect layer
        
    if detect_layer:
        nl = detect_layer.nl  # number of detection layers
        # Proceed with hyperparameter scaling
        hyp['box'] *= 3 / nl  # scale to layers
        hyp['cls'] *= nc / 80 * 3 / nl  # scale to classes and layers
        hyp['obj'] *= (imgsz / 640) ** 2 * 3 / nl  # scale to image size and layers
    else:
        raise AttributeError("No Detect layer found in the model.")
