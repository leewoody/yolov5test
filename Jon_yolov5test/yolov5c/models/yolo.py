...
class DetectionModel(BaseModel):
    # YOLOv5 detection model (original style) but with added debug
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):
        ...
        # Overwrite parameters if passed
...
class ClassificationModel2(BaseModel):
    # YOLOv5 classification model
    def __init__(self, cfg=None, model=None, cl=3, cutoff=10):  # yaml, model, number of classes, cutoff index
        super().__init__()
        self._from_detection_model(model, cl, cutoff) if model is not None else self._from_yaml(cfg)

    def _from_detection_model(self, model, cl=3, cutoff=10):
        # Create a YOLOv5 classification model from a YOLOv5 detection model
        if isinstance(model, DetectMultiBackend):
            model = model.model  # unwrap DetectMultiBackend
        model.model = model.model[:cutoff]  # backbone
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    if act:
        Conv.default_act = eval(act)  # redefine default activation, e.g. nn.SiLU()
    ...
    layers, save = [], []
    c2 = ch[-1]  # initial output channels (input to the first layer is ch[-1])
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):
        ...
        if isinstance(f, int):
            ...
            # If f is negative, Python's negative indexing is used (e.g., f = -1 -> last element)
            c1 = ch[f]
        else:
            # f is a list of indices, sum the corresponding channels
            c1 = sum(ch[x] for x in f)
        

        # Determine output channels (c2)
        if m in {
